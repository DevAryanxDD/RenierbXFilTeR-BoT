...
async def start(client, message):
    ...
    if len(message.command) != 2:
        ...
        reply_markup = InlineKeyboardMarkup(buttons)
        m=await message.reply_sticker("CAACAgUAAxkBAAEKeA9lINZIrBUr6JtIpQrUsKO6Z_6tvgACqAADyJRkFJWi9VCRb0zWMAQ") 
        await asyncio.sleep(1)
        await m.delete()
        n=await message.reply_sticker("CAACAgUAAxkBAAEKeBFlINZmOFOJcL2Aij9EyRXovfC1fQACKAEAAsiUZBQfHvy6bv2bUjAE") 
        await asyncio.sleep(1)
        await n.delete()
        await message.reply_video(
            video="https://telegra.ph/file/735b795058f7ef372a88c.mp4",
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        ...
        btn = [
            [
                ...
                InlineKeyboardButton('ᴡʜʏ ᴛᴏ ᴊᴏɪɴ ?🤔', callback_data='sinfo')
            ]
        ]
    ...
    if not files_:
        ...
        try:
            ...
            title = '@MovieZxWorld  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
    ...
    files = files_[0]
    title = '@MovieZxWorld  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            ...
            f_caption=f_caption
    if f_caption is None:
        f_caption = f"@MovieZxWorld  {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))}"
    if not await check_verification(client, message.from_user.id) and VERIFY == True:
        btn = [[
            InlineKeyboardButton("Verify", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start="))
        ]]

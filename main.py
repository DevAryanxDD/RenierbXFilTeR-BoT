...
async def init():
    ...
    @app.on_message(
        filters.command("mode") & filters.user(ADMINS)
    )
    async def mode_func(_, message: Message):
        if db is None:
            return await message.reply_text(
                "USERS_DB_URI var not defined. Please define it first"
            )
        usage = "**Usage:**\n\n/mode [group | private]\n\n**Group**: All the incoming messages will be forwarded to Log group.\n\n**Private**: All the incoming messages will be forwarded to the Private Messages of ADMINS"
        ...
        if state == "group":
            ...
            await message.reply_text(
                "Private Mode Enabled. All the incoming messages will be forwarded to Private Message of all ADMINS"
            )
        else:
            await message.reply_text(usage)

    @app.on_message(
        filters.command("block") & filters.user(ADMINS)
    )
    ...
    @app.on_message(
        filters.command("unblock") & filters.user(ADMINS)
    )
    ...
    @app.on_message(
        filters.group & ~filters.edited & filters.user(ADMINS),
        group=grouplist,
    )
